#include <stdint.h>
#include <stddef.h>
/* Full 64-step MD5 transform — dogfood oracle (separate from reduced md5_transform.c). */
typedef struct {
    uint32_t count[2];
    uint32_t state[4];
    uint8_t buffer[64];
...
void md5_transform_full_block(uint32_t state[4], const uint8_t block[64]) {
    uint32_t a=state[0],b=state[1],c=state[2],d=state[3],x[16];
    int i; for(i=0;i<16;i++) x[i]=((uint32_t*)block)[i];
    ...
    HH(c,d,a,b,x[15],16,0x1fa27cf8); HH(b,c,d,a,x[10],23,0xc4ac5665);
